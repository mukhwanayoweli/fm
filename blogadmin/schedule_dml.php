...
function schedule_insert(){
	...
	if(function_exists('schedule_before_insert')){
		$args=array();
		if(!schedule_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `schedule` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"schedule_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	if(function_exists('schedule_after_insert')){
		$res = sql("select * from `schedule` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
...
function schedule_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='schedule' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='schedule' and pkValue='$selected_id'");
...
//i will put a comment if it failes to run as i  wanted

	// child table: blogs
   /* $res = sql("select `id` from `schedule` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
...
function schedle_update($selected_id){
	global $Translation;
	...
	if(function_exists('schedule_before_update')){
		$args=array();
		if(!schedle_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `schedule` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
	...
	if(function_exists('schedule_after_update')){
		$res = sql("SELECT * FROM `schedule` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
...
function schedule_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `schedule` where `id`='".makeSafe($selected_id)."'", $eo);
...
$templateCode .= loadView('schedule-ajax-cache', $cache_data);
// hook: schedule_dv
